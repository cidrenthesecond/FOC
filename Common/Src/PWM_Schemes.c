...
{
	float _alpha = alpha/3.3;
	float _beta = beta/3.3;
...
#define ANGLE_TRANSFORMATIONS_AVAILABLE 4

typedef enum {
    ROTATE_MINUS_60_DEGREES = 0,
    ROTATE_MINUS_30_DEGREES,
    ROTATE_0_DEGREES,
    ROTATE_PLUS_30_DEGREES,
} RotateAngle;

static const float SIN_LUT[ANGLE_TRANSFORMATIONS_AVAILABLE] = {-0.8660254f,-0.5f,0.0f,0.5f};
static const float COS_LUT[ANGLE_TRANSFORMATIONS_AVAILABLE] = {0.5f,0.8660254f,1.0f,0.8660254f};

static inline int RotateVector(float alpha, float beta,
                               float *alpha_r, float *beta_r,
                               RotateAngle angle)
{
    *alpha_r = COS_LUT[angle] * alpha + SIN_LUT[angle] * beta;
    *beta_r  = -SIN_LUT[angle] * alpha + COS_LUT[angle] * beta;
