...
void ADC_Measure(int argc, char *argv[])
{
    ...
    switch (argument)
    {
        ...
        case 1:
        {
            ...
            char message[64];
            sprintf(message, "[ADC] : Phase A current : %u -> %d mA",measurementRaw,currents.Current_A);
            LOG(message);
            break;
        }
        case 2:
        {
            ...
            char message[64];
            sprintf(message, "[ADC] : Phase B current : %u -> %d mA",measurementRaw,currents.Current_B);
            LOG(message);
            break;
        }
        case 3:
        {
            ...
            char message[64];
            sprintf(message, "[ADC] : Phase C current : %u -> %d mA",measurementRaw,currents.Current_C);
            LOG(message);
            break;
        }
        case 4:
